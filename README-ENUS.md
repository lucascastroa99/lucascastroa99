<div align="right">
  <a href="https://github.com/lucascastroa99">
    <img src="https://raw.githubusercontent.com/HatScripts/circle-flags/gh-pages/flags/br.svg" width="25" alt="Português">
  </a>
</div>

<div align="center">

<img src="./memoji.png" width="500" alt="Lucas Castro Memoji">

![Visitor Count](https://komarev.com/ghpvc/?username=lucascastroa99&color=1eb955&style=for-the-badge&label=VISITORS)

# Lucas Castro

**Full Stack Developer** | React • Vue • Angular | Node.js • Python • .NET

[![Resume](https://img.shields.io/badge/Resume-4285F4?style=for-the-badge&logo=data:image/svg+xml;base64,&logoColor=white)](https://raw.githubusercontent.com/lucascastroa99/curriculum-vitae/main/LucasCastro_FullStackDeveloper_Resume-ENUS_02-02-2026.pdf)
[![Portfolio](https://img.shields.io/badge/Portfolio-1eb955?style=for-the-badge&logo=google-chrome&logoColor=white)](https://lucascastroa99.vercel.app)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=data:image/svg+xml;base64,&logoColor=white)](https://www.linkedin.com/in/lucascastroa99)
[![Email](https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white)](mailto:lucascastro.a99@gmail.com)

</div>

---

## 👨‍💻 About Me

Full Stack Developer with **over 4 years of experience** building robust and scalable web applications. Specialized in modern JavaScript frameworks (React, Vue, Angular) and backend with Node.js, Python, and .NET.

Currently working on enterprise-level applications, with expertise in **end-to-end projects**: from requirements gathering to CI/CD, performance testing, and UX/UI optimization. Committed to delivering high-impact solutions with focus on **performance and client value**.

## 🛠️ Tech Stack

### Frontend
<div align="left">
  <img src="https://skillicons.dev/icons?i=react,angular,vue,nextjs,nuxtjs,typescript,javascript,html,css,tailwind,bootstrap,materialui,jquery,styledcomponents&theme=light" alt="Frontend Skills" />
</div>

### Backend
<div align="left">
  <img src="https://skillicons.dev/icons?i=nodejs,python,dotnet,express,nestjs,django,flask,prisma,rabbitmq&theme=light" alt="Backend Skills" />
</div>

### Databases
<div align="left">
  <img src="https://skillicons.dev/icons?i=postgresql,mysql,mongodb,sqlserver&theme=light" alt="Database Skills" />
</div>

### DevOps & Tools
<div align="left">
  <img src="https://skillicons.dev/icons?i=docker,git,github,gitlab,azure,nginx,postman,figma,vscode,selenium&theme=light" alt="DevOps & Tools" />
</div>

### Testing
<div align="left">
  <img src="https://skillicons.dev/icons?i=jest&theme=light" alt="Testing Skills" />
</div>

## 💼 Professional Experience

### Mid-Level Frontend Developer
**Marttech - Software Development** | Toledo, PR (Remote)  
*July 2024 – August 2025*

- 📊 Optimized **MapBox** implementation, reducing loading time by **80%** and memory usage from **6GB to 2GB**
- 🎯 Led technical requirements gathering and system architecture design
- 💡 Implemented responsive interfaces in **React.js** following Material UI and usability guidelines
- 🔧 Refactored application architecture, fixing critical performance issues

### Mid-Level Development Analyst
**Ibrowse Consultoria e Informática** | Porto Alegre, RS (Remote)  
*August 2023 – January 2024*

- ⚙️ Configured **CI/CD** pipelines and managed **GitLab** repositories
- ✅ Created unit tests with **77% code coverage** using Karma
- 🏗️ Defined architecture and design patterns for **Angular** applications
- 🔄 Developed and fixed critical functionalities across multiple modules

### Information Technology Analyst
**ClearSale** | São Paulo, SP (Remote)  
*January 2022 – December 2022*

- 🌐 Developed full-stack features using **Vue.js**, **C#**, and **SQL Server**
- 🧪 Implemented unit tests to ensure code quality
- 📦 Actively participated in code reviews and continuous improvement processes

## 🎓 Education

**Bachelor's Degree in Software Engineering**  
Federal Institute of Amazonas (IFAM) | 2019 - 2023

## 📜 Certifications

**React.js** | Coodesh - May 2023  
Score: 78.3% of Accuracy

## 📊 GitHub Stats

<div align="center">

![](https://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=lucascastroa99&theme=dark)
![](https://github-profile-summary-cards.vercel.app/api/cards/stats?username=lucascastroa99&theme=dark)
![](https://github-profile-summary-cards.vercel.app/api/cards/productive-time?username=lucascastroa99&theme=dark)

</div>

## 🌍 Languages

- 🇧🇷 **Portuguese** — Native or Fluent (C2)
- 🇺🇸 **English** — Basic or Elementary (A2)

## 📫 Let's Connect

- 💼 Open to new opportunities and collaborations
- 📄 Resume: **[Download PDF](https://raw.githubusercontent.com/lucascastroa99/curriculum-vitae/main/LucasCastro_FullStackDeveloper_Resume-ENUS_02-02-2026.pdf)**
- 🌐 Portfolio: **[lucascastroa99.vercel.app](https://lucascastroa99.vercel.app)**
- 💬 LinkedIn: **[linkedin.com/in/lucascastroa99](https://www.linkedin.com/in/lucascastroa99)**
- 📧 Email: **[lucascastro.a99@gmail.com](mailto:lucascastro.a99@gmail.com)**

---

<div align="center">

**Thanks for visiting!** 🚀

</div>